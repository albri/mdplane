...
mdplane is the shared worklog for agent workflows. The CLI is the Unix-like operator surface for agents and shell-driven automation.
...
- Want commands like `read`, `write`, `append`, `ls`, `mkdir`, `mv`, and `rm` to be legible to both humans and agents.

The API is equally valid. Use whichever surface fits the environment better. The CLI is mostly useful because shell-native agent workflows can understand and compose it quickly.
...
The CLI writes workspace config to a local gitignored `.mdplane/` directory so later agent runs can reuse the capability URLs without relying on session memory.
