...
const colorConfig: Record<LogoVariant, { squareBg: string; textColor: string; shadowColor: string; iconColor: string }> = {
  default: { squareBg: COLORS.amber, textColor: COLORS.ink, shadowColor: COLORS.ink, iconColor: COLORS.ink },
  inverted: { squareBg: COLORS.amber, textColor: COLORS.white, shadowColor: COLORS.white, iconColor: COLORS.ink },
}
  ...
  return (
    <span className={cn('inline-flex items-center', sizes.gap, className)}>
      <span
        ...
        className={cn(sizes.square, 'origin-top flex items-center justify-center')}
        style={{
      ...
      {showWordmark && (
        <span
          className={cn('font-display font-bold tracking-tighter', sizes.text)}
...
export function LogoMark({
  ...
  return (
    <span
      className={cn(sizes.square, 'origin-top inline-flex items-center justify-center', className)}
      style={{
        backgroundColor: colors.squareBg,
        boxShadow: `${sizes.shadow} ${colors.shadowColor}`,
        transform: 'rotate(-2deg) perspective(100px) rotateX(-2deg)'
