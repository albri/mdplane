...
/**
 * Query parameters for GET /r/{key}/folders
 * @operationId listRootFolderContents
 */
export const zListRootFolderContentsQuery = z.object({
  // Maximum number of items to return
  limit: z.coerce.number().int().gte(1).lte(1000).optional().default(50),
  // Pagination cursor for fetching next page
  cursor: z.string().optional(),
  // Special action to perform. When set to `export`, returns root contents as a downloadable archive instead of JSON listing.
  action: z.enum(['export']).optional(),
  // Archive format (only used when action=export)
  format: z.enum(['zip', 'tar.gz']).optional().default("zip"),
  // Include all nested contents recursively
...
export type ListRootFolderContentsQuery = z.infer<typeof zListRootFolderContentsQuery>;
...
/**
 * Query parameters for GET /a/{key}/folders
 * @operationId listRootFolderContentsViaAppendKey
 */
export const zListRootFolderContentsViaAppendKeyQuery = z.object({
  // Maximum number of items to return
  limit: z.coerce.number().int().gte(1).lte(1000).optional().default(50),
  // Pagination cursor for fetching next page
  cursor: z.string().optional(),
...
export type ListRootFolderContentsViaAppendKeyQuery = z.infer<typeof zListRootFolderContentsViaAppendKeyQuery>;

/**
 * Query parameters for GET /a/{key}/folders/claims
 * @operationId listRootFolderClaims
 */
export const zListRootFolderClaimsQuery = z.object({
  // Filter by author identifier
  author: z.string().optional()
});

export type ListRootFolderClaimsQuery = z.infer<typeof zListRootFolderClaimsQuery>;
...
/**
 * Query parameters for GET /w/{key}/folders
 * @operationId listRootFolderContentsViaWriteKey
 */
export const zListRootFolderContentsViaWriteKeyQuery = z.object({
  // Maximum number of items to return
  limit: z.coerce.number().int().gte(1).lte(1000).optional().default(50),
  // Pagination cursor for fetching next page
  cursor: z.string().optional(),
...
export type ListRootFolderContentsViaWriteKeyQuery = z.infer<typeof zListRootFolderContentsViaWriteKeyQuery>;

/**
 * Query parameters for POST /w/{key}/folders/bulk
 * @operationId bulkCreateFilesInRootFolder
 */
export const zBulkCreateFilesInRootFolderQuery = z.object({
  // Return immediately with job ID for async processing
  async: z.enum(['true', 'false']).optional().default("false")
});

export type BulkCreateFilesInRootFolderQuery = z.infer<typeof zBulkCreateFilesInRootFolderQuery>;
