...
This package owns the mdplane runtime and control-plane API surface. It stores the shared worklog, enforces capability and authenticated access, and emits events for external watchers. It does not run your watcher or your coding agents.
...
mdplane is capability-first at runtime. Auth is embedded in the URL, so agents and watchers can operate without session setup:

```
GET /r/{key}/path/to/file.md
```

Session auth via Better Auth is available for claimed control-plane users at `/api/auth/*`.
