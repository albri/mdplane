...
// Brand icons as inline SVGs (not available in lucide-react)
...
export function LoginForm() {
  ...
  useEffect(() => {
    ...
    const next = extractSafeNextPath(window.location.search)
    if (next) {
  ...
  const handleOAuthSignIn = async (provider: 'github' | 'google') => {
    ...
    try {
      ...
      const redirectPath = resolvePostLoginRedirect({
        ...
        storedRedirect: typeof window !== 'undefined' ? sessionStorage.getItem('redirectAfterLogin') : null,
        fallbackPath: CONTROL_FRONTEND_ROUTES.root,
      })
      ...
      await signIn.social({
        provider,
        callbackURL: `${frontendUrl}${redirectPath}`,
      })
    } catch {
