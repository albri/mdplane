...
const EXCLUDED_ROUTE_PATTERNS = [
  // Internal admin routes are intentionally outside the public capability/API contract.
  /^(get|post|put|patch|delete) \/admin\//,
  /^post \/api\/v1\/admin\/smoke-api-key$/,
  // Raw websocket upgrade endpoint is checked separately from OpenAPI coverage.
  /^ws \/ws$/,
  // Path traversal guard routes are implementation details, not contract routes.
  /\/etc\*?$/,
  /\/etc\/\*$/,
  // Empty-key guard routes exist only to return INVALID_KEY for malformed capability URLs.
  /^get \/r\/\/\*$/,
  /^put \/w\/\/\*$/,
  /^delete \/w\/\/\*$/,
  /^get \/r\/\/folders\/\*$/,
  /^get \/a\/\/folders\/\*$/,
  /^get \/w\/\/folders\/\*$/,
  // These root-folder helpers exist only to return INVALID_REQUEST for root operations.
